...
/**
 * Cancela la reserva segun el dni del cliente y el codigo de sala
 */
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if((Integer) session.getAttribute("id_empleado")==null) {
				...
				try {
					boolean funciona=modRec.cancelarReservaPista(request.getParameter("dni"), modRec.getIdSala(request.getParameter("codigo")));
					
					if(funciona) {
						request.setAttribute("confirmacion", "Reserva cancelada correctamente");
					}
					else {
						request.setAttribute("error", "No se ha cancelado la reserva correctamente");
					}
